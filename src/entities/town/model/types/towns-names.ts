  export enum TownName {
    Charleston = 'Charleston',
    FortCaroline = 'Fort Caroline',
    StAugustine = 'St. Augustine',
    FloridaKeys	= 'Florida Keys',
    TampaWood = 'Tampa	Wood',
    PortStJoe = 'Port St. Joe',
    Pensacola	= 'Pensacola',
    Biloxi = 'Biloxi',
    NewOrleans = 'New Orleans',
    CorpusChristi = 'Corpus Christi',
    Tampico = 'Tampico',
    VeraCruz = 'Vera Cruz',
    VillaHermosa = 'Villa Hermosa',
    Campeche = 'Campeche',
    Sisal = 'Sisal',
    Cancun = 'Cancun',
    Belize = 'Belize',
    Roatan = 'Roatán',
    Providence = 'Providence',
    Cartagena = 'Cartagena',
    SantaMarta = 'Santa Marta',
    Maracaibo = 'Maracaibo',
    Gibraltar = 'Gibraltar',
    Coro = 'Coro',
    PuertoCabello = 'Puerto Cabello',
    Caracas = 'Caracas',
    PuertoSanto = 'Puerto Santo',
    Georgetown = 'Georgetown',
    Curacao = 'Curacao',
    Margarita = 'Margarita',
    PortOfSpain = 'Port of Spain',
    Grenada = 'Grenada',
    StLucia = 'St. Lucia',
    Barbados = 'Barbados',
    Martinique = 'Martinique',
    Guadeloupe = 'Guadeloupe',
    AntiguaBricks = 'Antigua	Bricks',
    StKitts = 'St. Kitts',
    StMartin = 'St. Martin',
    StThome = 'St. Thome',
    SanJuan = 'San Juan',
    SantoDomingo = 'Santo Domingo',
    Isabella = 'Isabella',
    Tortuga = 'Tortuga',
    PortAuPrince = 'Port-au-Prince',
    Santiago = 'Santiago',
    Gibara = 'Gibara',
    Trinidad = 'Trinidad',
    Havana = 'Havana',
    Evangelista = 'Evangelista',
    NombreDeDios = 'Nombre de Dios',
    Cayman = 'Cayman',
    PortRoyale = 'Port Royale',
    GrandBahama = 'Grand Bahama',
    Eleuthera	= 'Eleuthera',
    Nassau = 'Nassau',
    Andross = 'Andross',
    CatIsland = 'Cat Island',
    CharlesTowne = 'Charles Towne',
    TurkIslands = 'Turk Islands'
  };

// Charleston	Wood	Bricks	Textiles	Corn	Meat	North America
// Fort Caroline	Bricks	Textiles	Cotton	Dyes	Meat	North America
// St. Augustine	Bricks	Wheat	Sugar	Dyes	Clothing	North America
// Florida Keys	Bricks	Fruits	Corn	Tools	Dyes	North America
// Tampa	Wood	Wheat	Metal	Dyes	Clothing	North America
// Port St. Joe	Sugar	Textiles	Metal	Dyes	Bread	North America
// Pensacola	Textiles	Metal	Cotton	Dyes	Bread	North America
// Biloxi	Sugar	Corn	Metal	Dyes	Bread	North America
// New Orleans	Bricks	Sugar	Textiles	Dyes	Bread	North America
// Corpus Christi	Wood	Wheat	Metal	Dyes	Clothing	North America
// Tampico	Wood	Wheat	Sugar	Coffee	Clothing	Central America
// Vera Cruz	Bricks	Corn	Cotton	Tools	Coffee	Central America
// Villa Hermosa	Wheat	Sugar	Hemp	Coffee	Meat	Central America
// Campeche	Wood	Bricks	Corn	Coffee	Meat	Central America
// Sisal	Wheat	Metal	Cotton	Coffee	Meat	Central America
// Cancun	Corn	Sugar	Metal	Coffee	Meat	Central America
// Belize	Wood	Corn	Metal	Coffee	Clothing	Central America
// Roatán	Wheat	Sugar	Metal	Coffee	Bread	Central America
// Providence	Textiles	Metal	Cotton	Coffee	Ropes	Central America
// Cartagena	Wood	Sugar	Metal	Coffee	Meat	South America
// Santa Marta	Corn	Sugar	Hemp	Coffee	Meat	South America
// Maracaibo	Wood	Sugar	Cotton	Coffee	Meat	South America
// Gibraltar	Wood	Wheat	Hemp	Coffee	Clothing	South America
// Coro	Bricks	Wheat	Cotton	Cacao	Clothing	South America
// Puerto Cabello	Wheat	Metal	Cotton	Cacao	Ropes	South America
// Caracas	Bricks	Wheat	Sugar	Tools	Cacao	South America
// Puerto Santo	Wood	Hemp	Textiles	Cacao	Clothing	South America
// Georgetown	Wood	Bricks	Wheat	Tools	Cacao	South America
// Curacao	Wood	Bricks	Corn	Cacao	Meat	Lesser Antilles
// Margarita	Wood	Wheat	Hemp	Cacao	Bread	Lesser Antilles
// Port of Spain	Wheat	Sugar	Hemp	Tools	Cacao	Lesser Antilles
// Grenada	Fruits	Corn	Hemp	Cacao	Ropes	Lesser Antilles
// St. Lucia	Corn	Metal	Cotton	Cacao	Ropes	Lesser Antilles
// Barbados	Fruits	Textiles	Metal	Cacao	Ropes	Lesser Antilles
// Martinique	Wheat	Fruits	Hemp	Tools	Cacao	Lesser Antilles
// Guadeloupe	Bricks	Fruits	Corn	Cacao	Bread	Lesser Antilles
// Antigua	Bricks	Fruits	Textiles	Cacao	Ropes	Lesser Antilles
// St. Kitts	Fruits	Corn	Metal	Cacao	Rum	Lesser Antilles
// St. Martin	Bricks	Fruits	Corn	Cacao	Rum	Lesser Antilles
// St. Thome	Fruits	Corn	Cotton	Tobacco	Rum	Lesser Antilles
// San Juan	Fruits	Hemp	Textiles	Tobacco	Rum	Greater Antilles
// Santo Domingo	Bricks	Sugar	Textiles	Tobacco	Bread	Greater Antilles
// Isabella	Wood	Hemp	Textiles	Tobacco	Ropes	Greater Antilles
// Tortuga	Sugar	Hemp	Cotton	Tobacco	Rum	Greater Antilles
// Port-au-Prince	Bricks	Sugar	Textiles	Tools	Tobacco	Greater Antilles
// Santiago	Wood	Hemp	Cotton	Tools	Tobacco	Greater Antilles
// Gibara	Wheat	Metal	Cotton	Tobacco	Bread	Greater Antilles
// Trinidad	Wood	Sugar	Textiles	Tools	Tobacco	Greater Antilles
// Havana	Bricks	Wheat	Cotton	Tobacco	Rum	Greater Antilles
// Evangelista	Fruits	Hemp	Textiles	Tobacco	Ropes	Greater Antilles
// Nombre de Dios	Wood	Hemp	Textiles	Tobacco	Clothing	Greater Antilles
// Cayman	Fruits	Sugar	Hemp	Tools	Tobacco	Greater Antilles
// Port Royale	Corn	Metal	Cotton	Tobacco	Rum	Greater Antilles
// Grand Bahama	Bricks	Fruits	Corn	Tobacco	Rum	The Bahamas
// Eleuthera	Fruits	Hemp	Textiles	Tobacco	Ropes	The Bahamas
// Nassau	Fruits	Corn	Cotton	Tobacco	Bread	The Bahamas
// Andross	Wheat	Fruits	Cotton	Tobacco	Ropes	The Bahamas
// Cat Island	Fruits	Hemp	Textiles	Tobacco	Rum	The Bahamas
// Charles Towne	Fruits	Corn	Metal	Tobacco	Rum	The Bahamas
// Turk Islands
